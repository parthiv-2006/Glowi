 ...
 *
 * These are correlations, not causation — callers must always surface
 * CORRELATION_CAVEAT alongside the insights. Free of I/O; self-contained
 * types (no imports from mobile/) so it runs standalone in Deno.
 *
 * ⚠ Lockstep: mirror of mobile/src/lib/correlation.ts —
 * change both or neither.
 */

export type ChangeDirection = 'improved' | 'worsened' | 'unchanged';

/** Minimal shape of a completed scan, as read from the `scans` table. */
export interface CorrelationScan {
  status: string;
  skin_score: number | null;
  concerns: { concern_slug: string; display_name: string; severity: number }[];
  created_at: string;
}

/** Minimal shape of a shelf item, as read from the `shelf_items` table. */
export interface CorrelationShelfItem {
  name: string;
  key_ingredients: string[];
  created_at: string;
}

/** Minimal shape of a reaction log, as read from the `reaction_logs` table. */
export interface CorrelationReactionLog {
  product_name: string;
  key_ingredients: string[];
  reacted_on: string;
}
...
function completedAscending(scans: CorrelationScan[]): CorrelationScan[] {
  return scans
 ...
 */
export function cycleEvents(logs: CorrelationLifestyleLog[]): CorrelationEvent[] {
  const sorted = logs
    .filter(
      (l): l is CorrelationLifestyleLog & { cycle_phase: string } =>
