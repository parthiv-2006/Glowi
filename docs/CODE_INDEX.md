...
| `/learn` | `(tabs)/learn.tsx` | Articles, with a bookmark toggle and a "Saved" filter chip; reader at `article/[slug].tsx` (bookmark toggle mirrors the back button; a fixed reading-progress bar tracks scroll position via `ReadingProgressBar`, driven entirely on the UI thread alongside the parallax hero's `scrollY`) |
...
| `/results/[scanId]` | `results/[scanId].tsx` | Scan reveal; per-concern detail at `concern/[scanId]/[slug].tsx` (also shows a "What helped" card — `WhatHelpedCard` fed by `whatHelpedConcern`, filtering the same correlation insights the Progress tab computes down to this one concern) |
...
| `/routine` | `routine/index.tsx` | AM/PM routine, wait-time chips, order warnings, daily check-in; each step has a tap-to-edit note (`StepNoteEditor`, `updateRoutineStepNote`) saved independently of routine generation/editing |
| `/shelf` | `shelf/index.tsx` | Inventory + nudges, with search (name/brand/notes) + category filter chips + sort (attention/name/expiry/price) over the list; `add.tsx` (photo→AI→form), `[id].tsx` (detail), `budget.tsx` (cost-per-use), `conflicts.tsx` (AI conflict report), `replenish.tsx` (Smart Replenishment — what to get next; no-catalog-match falls back to a prefilled coach chat) |
| `/shelf/dupes/[id]` | `shelf/dupes/[id].tsx` | Dupe Finder — cheaper catalog alternatives that share key ingredients with one shelf item, ranked by overlap then savings; reached via "Find a cheaper dupe" on the shelf item detail screen |
...
| `/wishlist` | `wishlist.tsx` | Catalog products saved via a ProductCard heart toggle (concern detail, Replenish); filters the already-cached catalog client-side by wishlisted id, same pattern as Learn's favorite-slug matching |
...
`useLearnFavorites`/`useToggleLearnFavorite` (article bookmarks, optimistic toggle + rollback), `useWishlist`/`useToggleWishlist` (catalog-product wishlist, same optimistic toggle + rollback shape), …) |
...
**Feature (`components/`):** `CrashFallback` (what a render crash shows — Glowi + Restart, rendered by the `Sentry.ErrorBoundary` in `_layout.tsx`; works with or without a DSN), `AuroraBackground` (+`.web`), `BeforeAfterSlider`, `ConcernTrendSparkline`, `DailyCheckinCard` (Home lifestyle check-in — tap-to-upsert scales/chips, opt-in cycle row), `FaceZoneMap` (SVG face silhouette tinting the regions where scan concerns appeared, shaded by severity; on results + concern detail, additive over the existing area chips), `ConflictGraph` (SVG network of shelf products above the conflict cards on shelf → conflicts; severity by colour + dash + legend, tap an edge/node to select and scroll to its card), `CorrelationChart` (mini before/after SVG line chart under each correlation insight on Progress — dashed run-in, direction-coloured after segment, clay event pin, signed delta), `RoutineTimeline` (horizontal strip of step icons + proportional wait-time bars above the editable step list on the routine screen; static warnings-count flag, no tap target since `SequenceWarning` has no step attribution), `ReadingProgressBar` (thin fixed bar on the article reader; takes a shared value, not a plain number, so scroll doesn't force a JS re-render), `WhatHelpedCard` (past events that measurably improved one concern, on the concern detail screen; renders nothing when there's nothing to show), `StepNoteEditor` (tap-to-edit note on a routine step — view/edit/add-prompt, saves independently of the routine's own generate/edit flow), `GlowiAvatar` (the mascot — never draw a one-off), `Markdown`, `ProductCard` (optional `saved`/`onToggleSave` props render a wishlist heart toggle; absent when a call site doesn't wire it in), `ScoreTrend`, `ShelfItemCard`, `SkinWeatherCard`, `SplashView`, `TabBar`, `GlowReportShareCard` (4:5 share-safe branded card — static ring, no photos, captured via react-native-view-shot), `chat/` (MessageBubble, TypingDots), `scan/` (ScanTheater +`.web`).
...
| `mobile/src/{app,components}/**/__tests__/` | Component tests (RNTL): sign-up funnel · DailyCheckinCard (optimistic upsert + rollback) · chat send (draft restored on failure) · replenish (the `loading → error → empty → data` precedence; AI coach line replacing the deterministic rationale, F1) · dupe finder (same precedence; not-found route id; ranked dupe with savings rationale) · shelf search/filter (error/empty states, search narrowing the list, distinct no-matches state) · FaceZoneMap (severity-word a11y label · unmapped-area fallback · null when nothing maps) · RoutineTimeline (summary a11y label with step/wait/warning counts · per-node wait label · null under two steps) · Learn bookmark toggle (optimistic + rollback) · Wishlist (error/empty states, catalog filtered to wishlisted ids) · ProductCard wishlist toggle (heart absent without `onToggleSave`, saved/unsaved a11y contract, tap calls through) |
