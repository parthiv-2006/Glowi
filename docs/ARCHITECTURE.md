...
| `check-conflicts` | Filters the user's active shelf items down to those with known `key_ingredients`; if a cached `conflict_reports` row is at least as new as the latest shelf change, returns it with no Claude call. Otherwise asks Claude (temperature 0) for strict-JSON ingredient interactions, **validates** each conflict field-by-field via `sanitizeConflicts` (severity enum, capped/typed fields, malformed entries dropped individually), caches, and returns the report ([ADR-0008](adr/0008-ingredient-conflict-checker.md)). |
...
| `compare-scans` | Claude vision diff between two completed scan photos — an honest before/after delta for the Progress tab. **Validates** the response server-side and persists one immutable row per pair in `scan_comparisons`, so repeating a comparison never re-pays the Claude call. |
| `glow-report` | The Weekly Glow Report: gathers a completed week's real data (scan movement, routine adherence, shelf adds, reactions, environment, correlations), asks Claude for the report's prose, **validates** it field-by-field, and inserts one immutable `glow_reports` row — idempotent per user per week ([ADR-0014](adr/0014-weekly-glow-report.md)). |
...
| `auth-signup` | Creates pre-confirmed email and guest users via the admin API, and converts a calling guest in place (mode `upgrade` — same user id, so all RLS-owned data survives) ([ADR-0002](adr/0002-prefilled-auth-signup-function.md)). |
...
| `delete-account` | Permanent in-app erasure (Apple 5.1.1(v), GDPR/CCPA): authenticated by the caller's own JWT, drains the user's private storage prefix first, then `auth.admin.deleteUser` cascades every DB row. Fails closed — a partial storage failure aborts so photos are never orphaned ([ADR-0019](adr/0019-account-deletion.md)). |
| `cleanup-guests` | Monthly pg_cron sweep (same Vault-secret auth as `push-dispatch`) deleting guest accounts with no activity for 90 days, capped per run, with a `dryRun` mode that only reports. Storage is removed before the auth user; a storage failure skips the user for retry next run ([ADR-0018](adr/0018-abandoned-guest-cleanup.md)). |
